...
def fly():
    ...
    while running:
        print("stage=", stage)
        keyPressed, stage, running, flying = perform_manual_intervention(
            stage, running, flying)
        ...
        if (frame != None):
            if (keyPressed == False):
                if (stage == 1):
                    (success, frame, counter, centerX, centerY, radius, inbox_width_and_height) = circleDetectorClass.detectCircle(
                        counter, frame, centerX, centerY, radius)
                    ...
                    if success:
                        ...
                        camShiftHandler = camShiftClass.CamShift(
                            centerX, centerY, inbox_width_and_height, inbox_width_and_height, frame, follow_light, useOpenCV3)

                        
                        success, frame, centerPtX, centerPtY, minWidth = camShiftHandler.performCamShift(
                            frame)
                elif (stage == 2):
                    print("stage =2")
                    success, frame, centerPtX, centerPtY, minWidth = camShiftHandler.performCamShift(
                    ...
                    # if object is farer away than at the starting time, then the ratio is > 0
                    edgeLengthRatio = currentEdgeLength/initialEdgeLength
                    cv2.putText(frame, "Size ratio: " + str(edgeLengthRatio),
                    ...
                    if success:
                        ...
                        if centerPtX < windowCenterX - 50:
                            ratio_normalized = (
                                windowCenterX-centerPtX/diffX_max)
                            ...
                            ratio_normalized = (
                                centerPtX-windowCenterX/diffX_max)
                            # drone.speed = MAX_SPEED_ROT * ratio_normalized
                            return_to_hover = True
                            drone.turn_right()
                            print("turn right")
                        ...
                        if (edgeLengthRatio > 1.2):
                            ratio_normalized = (
                                edgeLengthRatio-1)/(max_ratio_window_with_to_edge-1)
...
def show_frame(frame):
        cv2.putText(frame, "Press 'x' to go to circleDetection: ",
                    (170, 450), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0))
